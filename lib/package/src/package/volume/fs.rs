...
mod tests {
    ...
    fn fs_volume_preserves_symlinks() {
        ...
        let absolute_target = target_file.to_str().unwrap().to_owned();
        ...
        std::os::unix::fs::symlink(&absolute_target, temp.path().join("absolute-link")).unwrap();
        ...
        let absolute_link: PathSegments = "/absolute-link".parse().unwrap();
        assert_eq!(volume.read_link(&absolute_link).unwrap().0, absolute_target);
        ...
        for link in ["absolute-link", "broken-link", "dir-link", "file-link"] {
            assert!(matches!(
