...
mod tests {
    ...
    fn serialize_package_with_symlinks() {
        ...
        let absolute_target = target.to_str().unwrap().to_owned();
        ...
        std::os::unix::fs::symlink(&absolute_target, assets.join("absolute-link")).unwrap();
        ...
        assert_eq!(
            volume.read_link("/absolute-link").unwrap().0,
            absolute_target
        );
        assert_eq!(
